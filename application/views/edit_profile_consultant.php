	...
	<nav class="navbar navbar-expand-lg">
		...
		<div class="collapse navbar-collapse" id="navbarText">
			...
			<span class="navbar-text">
				<a class="nav-link <?= $logged_in ? '' : 'modal-login' ?>"
					href="<?=$logged_in ? ($role == 'consultants' ? 'showprofile' : 'showprofile') : '#' ?>"><?= $logged_in ? ($role == 'consultants' ? $profile->namaConsultant : $profile->namaPasien) : 'Login' ?>
		...
		<form action="<?=base_url('Consultant/setUpdateProfile')?>" method="POST" enctype='multipart/form-data'>
			<div class="row">
				<div class="col-md-4">
					...
					<?php }else{ ?>
					<img class="photoprofile" src="<?=base_url('assets/img/Consultants/').$profile->photo?>" width:200px alt="photoprofile">
					<?php }?>
				...
				<div class="col-md-8">
					...
					<input type="number" name="noHP" class="form_edit_profile_consultant" value="<?=$profile->noHP ?>">
