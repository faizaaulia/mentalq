...
<body style="background-image: url('<?= base_url('assets/img/BG ABU.jpg')?>');">
	...
	<?php
		...
		if (!empty($notifsukses)) 
			echo "<div class='alert alert-success mb-3' style='width: 50%; margin: auto; display: block'>$notifsukses</div>";
		if (!empty($notiferror)) 
			echo "<div class='alert alert-danger mb-3' style='width: 50%; margin: auto; display: block; text-align: center'>$notiferror</div>";
	?>
	...
	<form id="consult" action="<?= base_url('Patient/inputKeluhan')?>" method='post'>
		...
		<select name="ConsultantList" style="margin: auto; display: block">
			...
			<option value="<?= $consultant->noSTR ?>">
				<?= $consultant->namaConsultant ?> ( <?= $consultant->jamKerja ?>, <?= $consultant->schedule ?>, <?= $consultant->tempatPraktik ?> )
			</option>
			<?php endforeach ?>
		</select>
		<br>
